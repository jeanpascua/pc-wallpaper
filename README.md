...
Live hacker-terminal-themed desktop wallpaper. Renders real-time CPU/GPU/RAM/storage stats to a PNG and sets it as your actual GNOME wallpaper (~1s refresh), with color-coded thresholds (green/amber/red) so you can read system health at a glance.

## Compatibility (limited — read before installing)

This is a personal project built for one specific machine and is **not** broadly portable yet.
...
| Desktop | GNOME (Wayland or X11, via `gsettings`) | KDE/XFCE/other — raises `NotImplementedError` |
...
- `main.py` — persistent loop: collect stats → render → set as wallpaper via `gsettings`, once per second
